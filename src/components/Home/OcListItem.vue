<template>
	<div
		...
		:style="styleObj"
	>
		<router-link :to="linkTo" class="inner block box-border p-3 relative h-full">
			<h3
			...
			<div class="media flex items-center absolute bottom-4 max-w-full">
				...
				>
					<img :src="collection?.proposer.avatar" alt="user avatar
					" class="w-full" />
				</span>
				<div class="content flex flex-col max-w-full ml-1.5 pr-4 overflow-hidden">
					<div
						class="name max-width-100% font-normal overflow-hidden overflow-ellipsis whitespace-nowrap text-footer"
					>{{ collection?.proposer.name }}</div>
