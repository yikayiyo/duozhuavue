	...
	<router-link :to="/books/ + id">
		<div class="douban-style-book-wrapper px-3.75 pt-3.75">
			<div class="top flex justify-between">
				<div
					class="image-wrapper rounded w-23 mr-2 h-32 border border-menu dark:border-darkbg bg-menu dark:bg-darkbg"
				>
					<div
				...
				<div
					class="book-intro-wrapper rounded flex-1 bg-menu dark:bg-darkbg px-5 pt-6 text-cbl leading-1.6 flex flex-col justify-evenly"
				>
			...
			<div class="middle flex justify-between items-center py-2">
				<div class="left max-w-1/2">
					<div class="title text-base whitespace-nowrap overflow-hidden overflow-ellipsis">{{ title }}</div>
				</div>
				<div class="right">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-7.5" :fill="'red'">
						<path
			...
			<div
				class="bottom text-xs text-sold-out py-2 border-b-0.5 dark:border-darkborder overflow-hidden overflow-ellipsis whitespace-nowrap"
			>{{ bookInfo }}</div>
		</div>
	</router-link>
</template>
