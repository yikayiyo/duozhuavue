<template>
	<div
		class="
			fixed
			max-w-dzy
			mx-auto
			my-0
			bottom-0
			inset-x-0
			border-t border-b
			z-50
			bg-white
		"
	>
		<ul class="flex flex-row justify-between items-center">
			<li class="flex-1 flex flex-col items-center justify-center">
				<router-link class="flex flex-col items-center" to="/book">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 200 200"
						style="height: 30px; width: 30px; fill: rgb(51, 51, 51)"
					>
						<path
							d="M151.34 170.38H48.16c-5.78 0-10.38-3.74-12.28-10-4.55-15-.68-54.86-.22-59.37-.13-.81-1.32-1.67-2.9-1.67a12.36 12.36 0 01-12-8.08 10.78 10.78 0 014-12.12l67-50.5a13.47 13.47 0 0115.94 0l67 50.5a10.78 10.78 0 014 12.12 12.35 12.35 0 01-12 8.08c-1.57 0-2.77.86-2.9 1.67.45 4.41 4.16 43-.17 59.21-1.63 6.27-6.35 10.16-12.29 10.16zM99.75 35.61a3.65 3.65 0 00-2.19.67l-67 50.51c-.91.68-.74 1.2-.66 1.42.25.76 1.23 1.53 2.86 1.53 6.89 0 12.5 5.12 12.5 11.4v.5c-1.22 11.71-3.63 44.59-.18 55.94 1 3.22 2.53 3.22 3.11 3.22h103.15c1.5 0 2.53-1 3.07-3.06 4-15.15-.09-55.69-.14-56.1v-.5c0-6.28 5.6-11.4 12.49-11.4 1.63 0 2.62-.77 2.87-1.53.07-.22.25-.74-.67-1.42L102 36.28a3.7 3.7 0 00-2.25-.67zm-5.08-3.16z"
						></path>
						<path
							d="M80.1 153.37a4.79 4.79 0 01-4.79-4.8v-19.32c0-7.75 7.61-16.06 17-16.06h14.94c9.36 0 17 8.31 17 16.06v19.32a4.8 4.8 0 11-9.59 0v-19.32c0-2.11-3.16-6.47-7.39-6.47H92.28c-4.23 0-7.39 4.36-7.39 6.47v19.32a4.79 4.79 0 01-4.79 4.8z"
						></path>
					</svg>
					<span class="text-xss">首页</span>
				</router-link>
			</li>
			<li class="flex-1 flex flex-col items-center">
				<router-link class="flex flex-col items-center" to="/sell">
					<svg viewBox="0 0 200 200" style="height: 30px; width: 30px">
						<path
							d="M48.21 41.39a3.55 3.55 0 10-5.78 4.14l3.17 4.41a3.55 3.55 0 105.77-4.14z"
						></path>
						<path
							d="M171.51 85.56C166.16 72.9 154.69 70.92 146.32 72c-2.75-13.71-11.89-20.57-20.21-22.43-17-3.76-27.68 7.34-32.64 16.79a40 40 0 00-3.71-1.68C88.92 54.4 87.27 44 84 34.33a50.34 50.34 0 00-8.7-16.28c-4.48-5.34-9.75-5-16-3.66a225.14 225.14 0 00-45.46 14.66 4 4 0 00-2.54 3.11 4.15 4.15 0 00.64 3.48c12.74 19.58 19.44 44.71 14.19 67.85-.64 2.85 1.18 6.39 4.53 6.24 5.42-.24 10.84-.68 16.23-1.31a22.41 22.41 0 00-4.39 19.39c2.28 9.08 9.55 19.95 30.79 22.14a194.22 194.22 0 0012 26.57 4.84 4.84 0 008.4-4.83c-.06-.11-13.48-31-13.55-31.14a4.83 4.83 0 00-8.87-.64c-10.91-1.74-17.44-6.58-19.42-14.46S56.1 110.74 63 109l6.78-1.75-1.67-2.39-2.34-3.35a25.41 25.41 0 01-3-5.45c-4.39-11.37 3.85-19 4.94-19.94a16.85 16.85 0 0112.79-3.8 25 25 0 019.84 3.44c.85.48 1.71 1 2.59 1.64l5.33 3.69 2-6.16c.26-.81 6.75-19.62 23.67-15.85 2.06.45 12.37 3.53 13.37 19.19l.43 6.35 6-2.09c1.41-.49 14-4.47 18.75 6.81 3.87 9.17 1.74 16.45-6.88 23.27a5.1 5.1 0 00-1.08.19 6.12 6.12 0 00-2.79 8.47 4.63 4.63 0 00.71.81 108.71 108.71 0 017.23 33.92 4.85 4.85 0 004.73 4.65h.31a4.85 4.85 0 004.65-5 117.55 117.55 0 00-7.29-35.65c14.61-12 13.34-25.17 9.44-34.44zM67.5 64.88a3.63 3.63 0 00-2.33-.48l-5.58.8-.9-6.31 7.66-1.1a3.56 3.56 0 10-1-7l-5 .72L64.2 44a3.56 3.56 0 00-6.3-3.3l-5.5 10.54a3.52 3.52 0 00-.4 1.42L43 54a3.56 3.56 0 00.39 7 4 4 0 00.61 0l7.67-1.1.9 6.31L47 67a3.55 3.55 0 00.42 7.07A4 4 0 0048 74l5.58-.8.66 4.8c-3.18 6.8-2.71 13.91-.91 19.76Q45 99.08 36.63 99.7c3.16-22-2.27-45-13.39-64.17A214 214 0 0160.13 24.1c3.18-.68 5.86-2.06 8.16.64a35.13 35.13 0 015.32 9.34c2.46 6 3.73 12.45 4.8 18.8.55 3.21 1 6.43 1.33 9.66a26.74 26.74 0 00-12.24 2.34z"
						></path>
					</svg>
					<span class="text-xss">卖东西</span>
				</router-link>
			</li>
			<li class="flex-1 flex flex-col items-center">
				<router-link class="flex flex-col items-center" to="/cart">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 200 200"
						style="height: 30px; width: 30px"
					>
						<path
							d="M53.69 47.63a4.88 4.88 0 004.81 4.94h.06a4.87 4.87 0 004.87-4.81 25.52 25.52 0 00-8.76-18.26c-8.35-7.29-16.62-7.29-34.73-7.29a4.87 4.87 0 000 9.74c17.64 0 23 .21 28.32 4.89a15.65 15.65 0 015.43 10.79zM67.64 141.55a15.19 15.19 0 1015.19 15.19 15.21 15.21 0 00-15.19-15.19zm0 20.64a5.45 5.45 0 115.45-5.45 5.46 5.46 0 01-5.45 5.45zM138.89 141.55a15.19 15.19 0 1015.19 15.19 15.21 15.21 0 00-15.19-15.19zm0 20.64a5.45 5.45 0 115.44-5.45 5.45 5.45 0 01-5.44 5.45zM167.35 62.79a18.44 18.44 0 00-13.86-6.1h-103a18.44 18.44 0 00-13.86 6.1 16.11 16.11 0 00-4 12.79L36 103.25c2.33 19.09 16.33 30.48 37.45 30.48h57.11c21.12 0 35.12-11.39 37.46-30.48l3.37-27.67a16.11 16.11 0 00-4.04-12.79zm-5.62 11.61l-3.38 27.67C156.63 116.2 146.76 124 130.57 124H73.46c-16.19 0-26.06-7.79-27.78-21.92L42.3 74.4a6.51 6.51 0 011.7-5.15 8.73 8.73 0 016.57-2.82h103a8.73 8.73 0 016.57 2.82 6.51 6.51 0 011.59 5.15z"
						></path>
					</svg>
					<span class="text-xss">购物车</span>
				</router-link>
			</li>
			<li class="flex-1 flex flex-col items-center">
				<router-link class="flex flex-col items-center" :to="/users/ + userId">
					<svg
						viewBox="0 0 200 200"
						style="height: 30px; width: 30px; fill: rgb(73, 119, 73)"
					>
						<path
							d="M187.3 67a5.67 5.67 0 00-5.09-2.66c-2.18.14-8 3.58-12.1 5.15-4.91-14.77-20.72-24.17-27.22-26.68-1.17-5-4.21-14.16-8.92-17.29-1.93-1.28-2.68-.84-3.89-.72-3.94.41-10.57 10.25-13.33 15.22l-35.3.58c-2.29-6-9.07-14.45-13.42-14.45-3.51 0-14.65 14.71-16.6 22.74-5.48 1.44-14.69 7.26-23.91 24-4.52 1.85-15.8 6.33-18.81 8.45-1.64 1.16-2.26 2.11-2.06 4.09.22 2.25.55 5.57 21.35 15.13-.15 11.15 3.28 20.59 10.23 28.09a45.23 45.23 0 009.67 7.76C34.42 146.15 35 164 35.05 164.79v3.49h71.84s66.38.71 66.72 0 0-5.47 0-5.47c-.14-1.06-3.15-22.91-16.72-30.95a45.79 45.79 0 0016.21-28.67c2.43 0 5.79 3 10 2.52a4.88 4.88 0 003.82-2.65c4.15-8.44 3.83-30.85.38-36.06zm-99.14 1.68v.07a6 6 0 11.06-.66 3.71 3.71 0 010 .48.37.37 0 01-.06.11zM114 73.81a6.23 6.23 0 116.22-6.22 6.22 6.22 0 01-6.22 6.22z"
						></path>
					</svg>
					<span class="text-xss">我的</span>
				</router-link>
			</li>
		</ul>
	</div>
</template>

<script>
export default {
	name: "UserFooter",
	props: ["userId"],
};
</script>

<style></style>
