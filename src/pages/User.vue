<template>
  ...
  <div v-else class="user-wrapper text-shiwu leading-category">
    ...
    >
      ...
      <button
        @click="logOut"
        class="m-3.75 rounded-full border-0.5 px-3 py-1 text-xs text-white shadow-sm dark:border-darkborder"
      >
        登出帐号
      </button>
    </div>
    <div class="user-header-wrapper wrapper-bg text-white">
      <div class="header-under"></div>
      <div class="user-head">
        <div class="basic-info flex flex-col items-center pb-4.75 pt-8">
          <div
            class="avatar avatar-shadow h-20 w-20 overflow-hidden rounded-99 border-2 border-white dark:border-darkborder"
          >
            <img :src="user.avatar" alt="avatar image" class="w-full" />
          </div>
          <div
            class="name mx-3.75 my-2 max-w-1/2 overflow-hidden overflow-ellipsis whitespace-nowrap text-xl font-normal"
          >
            {{ user.name }}
          </div>
        ...
        <div
          v-if="showInfos"
          class="info-tab flex justify-around border-b-0.5 border-t-0.5 border-info dark:border-darkborder"
        >
