<template>
	...
	<div v-else class="container-wrapper text-is-active dark:text-menu relative">
		<div class="book-wrapper pb-12.75">
			<div class="book-header relative px-3.75 pt-7.5 pb-7 overflow-hidden flex flex-col items-center">
				<div class="image-wrapper absolute -inset-50 z-10" :style="imageWrapperStyle"></div>
				<div class="image w-37.5 h-50 relative z-20">
					<img class="w-full h-full" :src="book.image" :alt="book.title + '封面'" :style="imageStyle" />
				</div>
			</div>
			<div class="book-main pt-4.5 mx-3.75">
					...
					<div class="p-wrapper mt-3.5 text-hsh leading-1.69">
						...
						<p class="author flex">
							...
							<span class="ml-3 text-load">
								<router-link :to="'/search/book/' + book.rawAuthor">{{ book.rawAuthor }}</router-link>
							</span>
						</p>
				...
				<div class="book-detail-wrapper mt-3.75 pt-2.5 pb-3.75">
					...
					<div class="book-detail mb-7.5">
						<div
							class="content text-shiwu leading-1.8 break-words whitespace-pre-wrap"
							:class="collapsed ? 'collapsed' : ''"
						>
							<div class="book-summary mb-1.6em" v-html="book.summary"></div>
							<div class="book-author-intro mb-1.6em" v-html="book.authorIntro"></div>
							<div class="book-catalog" v-html="book.catalog"></div>
						</div>
						<div
							class="read-more flex text-hsh text-load mt-3 py-1.25 px-1em items-center justify-center"
						>
							<button @click="toggleCollapsed">{{ collapsed ? "查看更多" : "收起" }}</button>
							<svg
								v-if="collapsed"
								viewBox="0 0 24 24"
								fill="none"
					...
					<div class="book-comment-wrapper">
						<div class="comments-header flex justify-between">
							...
							<div
								v-else
								class="update-comment flex items-center text-load rounded-99 border-0.5 border-rating dark:border-darkborder box-border"
								@click="updateComment(myComment[0].id)"
							>
								<span
									class="avatar-wrapper inline-block w-7.5 h-7.5 rounded-99 border-1 border-rating dark:border-darkborder bg-menu dark:bg-darkbg overflow-hidden -m-1 mr-1"
								>
									<img :src="myComment[0].commenter.avatar" alt="comment avatar" />
								</span>
							...
							>
								<div class="comment-left">
									<router-link :to="'/users/' + comment.commenter.id">
										<span
											class="avatar-wrapper inline-block w-7.5 h-7.5 rounded-99 border-1 border-menu dark:border-darkborder bg-menu dark:bg-darkbg overflow-hidden"
										>
											<img :src="comment.commenter.avatar" alt="comment avatar" />
										</span>
									</router-link>
								</div>
								<div class="comment-right min-w-0 flex-1 ml-3 text-shiwu">
									<div class="name-with-rating flex mb-1.75 items-center">
										<p
											class="font-medium max-w-1/2 whitespace-nowrap overflow-hidden overflow-ellipsis"
										>{{ comment.commenter.name }}</p>
									...
									<div class="comment-content">
										...
										<div
											class="text-right text-xs text-sold-out"
										>{{ commentDateFormatter(comment.createdAt) }}</div>
									</div>
									<footer class="contrib-footer">
										<div class="interact-actions flex">
											<div class="comment-item" style="margin-right: 40px">
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 17">
													<g
														fill="none"
														fill-rule="evenodd"
														stroke="#AAA"
											...
											<div class="action-item">
												...
												<span class="label" v-if="comment.voters.length > 0">{{ comment.voters.length }}</span>
											</div>
										</div>
									</footer>
								</div>
